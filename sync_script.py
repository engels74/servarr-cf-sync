...
class CustomFormatSyncer:
        ...
        try:
            ...
            for filename, format_data in custom_formats.items():
                try:
                    ...
                    stored_version = self.version_manager.versions.get(
                        filename, semver.VersionInfo.parse("0.0.0")
                    )
                    ...
                    if file_version > stored_version or file_version < latest_version:
                        if file_version < latest_version:
                            logging.warning(
                                f"{filename} version {file_version} is behind latest version {latest_version}"
                            )

                        for instance_name, url, api_key in instances:
                            if not self.should_sync_to_instance(format_data, instance_name):
                                logging.info(
                                    f"Skipping {filename} for {instance_name} based on cfSync settings"
                                )
                                continue
                            ...
                            try:
                                ...
                                synced_format = self.sync_format(
                                    client, existing_formats, formatted_custom_format
                                )
                                ...
                                logging.error(
                                    f"Error syncing {filename} to {instance_name}: {str(e)}"
                                )
                                continue
    ...
    def should_sync_to_instance(self, format_data: FormatData, instance_name: str) -> bool:
        if "cfSync_instances" in format_data:
            ...
            instance_type: Literal["radarr", "sonarr"] = (
                "radarr" if "Radarr" in instance_name else "sonarr"
            )
    ...
    def prepare_format_for_sync(self, format_data: FormatData) -> FormatDict:
        ...
        result["includeCustomFormatWhenRenaming"] = format_data.get(
    ...
    def sync_format(
        self, client: APIClient, existing_formats: list[CustomFormat], new_format: FormatDict
    ) -> CustomFormat | None:
        existing_format = next(
            (f for f in existing_formats if f.get("name") == new_format.get("name")), None
        )
        ...
        try:
            ...
            for profile in profiles:
                ...
                if updated:
                    ...
                    logging.info(
                        f"Updated score for {format_name} in profile {updated_profile['name']}"
                    )
...
def main() -> None:
    ...
    if not instances:
        logging.error(
            "No Radarr or Sonarr instances configured. Please check your environment variables."
        )
        sys.exit(1)
